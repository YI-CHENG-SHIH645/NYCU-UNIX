  ...
  if (format != NULL && stbuf != NULL) {
    for (p = format; *p != '\0'; p++) {
      ...
      case 'G':
        ...
        i += sprintf(out + i, "%s", (grp != NULL) ? grp->gr_name : "");
        break;
      ...
      case 'U':
        ...
        i += sprintf(out + i, "%s", (pwp != NULL) ? pwp->pw_name : "");
        break;
...
void lsmodes(char *retbuf, int mode) {
  int ifmt = mode & __S_IFMT;

  if (ifmt == __S_IFDIR)
    retbuf[0] = 'd';
  else if (ifmt == __S_IFCHR)
    retbuf[0] = 'c';
  else if (ifmt == __S_IFBLK)
    retbuf[0] = 'b';
  else if (ifmt == __S_IFLNK)
    retbuf[0] = 'l';
  else if (ifmt == __S_IFSOCK)
    retbuf[0] = 's';
  else if (ifmt == __S_IFIFO)
    retbuf[0] = 'p';
  else
    retbuf[0] = '-';
  ...
  if (mode & S_ISUID)
    retbuf[3] = (mode & __S_IEXEC) ? 's' : 'S';

  if (mode & S_ISGID)
    retbuf[6] = (mode & (__S_IEXEC >> 3)) ? 's' : 'S';

  if (mode & __S_ISVTX)
    retbuf[9] = (mode & (__S_IEXEC >> 6)) ? 't' : 'T';
